...
SDL_Surface* Video::loadTexture(char* name) {
	SDL_Surface* textura = IMG_Load(name);
	...
	if ((!textura) || (textura->format->BitsPerPixel <= 8)) {
		...
		char message[100]; sprintf( message, "Numar de BPP invalid : %d", textura->format->BitsPerPixel );
		SDL_FreeSurface( textura );

		throw( std::logic_error( message ) );
	}

	std::cout << name << " = " << (int) textura->format->BitsPerPixel << "bpp" << std::endl;
...
void Video::makeTexture( SDL_Surface* surface, GLuint &texture ) {
	...
	glTexImage2D( GL_TEXTURE_2D, 0, surface->format->BytesPerPixel, surface->w, surface->h, 0, format,
		GL_UNSIGNED_BYTE, surface->pixels );
}

void Video::initFont() {
	if ((fontTexture) || (base)) return ;

	SDL_Surface* textura = loadTexture((char *) "data/fonturi/font.png");
...
void Video::print( GLint x, GLint y, char *string, int set ) {
	glBindTexture( GL_TEXTURE_2D, fontTexture );
...
void Video::drawTexture(int x, int y, int w, int h, GLuint texture, GLfloat yamount) {
	vertex2i v0 = { x, y };
	vertex2i v1 = { x, y + h };
	vertex2i v2 = { x + w, y };
	vertex2i v3 = { x + w, y + h };
    ...
    glBegin(GL_TRIANGLE_STRIP);
		glTexCoord2f( 0.0f, yamount ); glVertex2iv( v0 );

		glTexCoord2f( 0.0f, 1.0f ); glVertex2iv( v1 );

		glTexCoord2f( 1.0f, yamount ); glVertex2iv( v2 );
		
		glTexCoord2f( 1.0f, 1.0f ); glVertex2iv( v3 );
	glEnd();
}

Video::~Video() {
	if (base) glDeleteLists( base, 256 );
